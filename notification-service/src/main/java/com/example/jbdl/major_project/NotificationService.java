        ...
        if(ACTOR_SENDER_ATTRIBUTE.equals(actor)){
            switch(status){
                case TRANSACTION_SUCCESS_STATUS:
                    message = "Transaction with ID "+ transactionId + " has been completed at " + date + ".Your account has been debited by amount "+ amount + "Rs." ;
                    break;
                case TRANSACTION_FAILED_STATUS:
                    message="Transaction with ID "+ transactionId + " got failed"+ date + ". Please try again.";
                    break;
                default:
                    throw new Exception("Invalid status found");
            }
        }else{
            message="Your account has been credited with amount  " + amount + "Rs on " + date.toString();
        }

        simpleMailMessage.setText(message);
        simpleMailMessage.setTo(email);
        simpleMailMessage.setFrom("ewallet.notifications@gmail.com");
